...
class _SmartAppBarState extends State<SmartAppBar>
  ...
  void _updateCaches(ThemeData theme, bool isDark) {
    ...
    if (_cachedRouteName != routeName || _cachedConfig?.title != config.title) {
      ...
      _cachedBackgroundColor = widget.backgroundColor ?? config.backgroundColor ?? _getDefaultBackgroundColor(config, isDark);
      _cachedForegroundColor = widget.foregroundColor ?? config.foregroundColor ?? _getDefaultForegroundColor(config, isDark);
    }
  }
}